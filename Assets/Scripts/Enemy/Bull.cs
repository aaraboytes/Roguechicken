...
public class Bull : Enemy
{
    ...
    void Shoot()
    {
        if (canShootAgain)
        {
            ...
            {
                shotFuncs.Spike(bullet, (Vector2)transform.position + dir * 1.05f, dir, bulletForce,30);
            }
            else
            {
                shotFuncs.Circunference(bullet, transform.position, bulletForce, -bulletAcc, 15, 2, true);
            }
            canShootAgain = false;
